...
export function RadioGender({ name, value, label, selected, onSelect, icon }: RadioGenderProps) {
  return (
    <button
      ...
      onClick={onSelect}
      className={`flex flex-col items-center gap-2 p-3 rounded-[16px] bg-transparent focus:outline-none ${
        selected ? 'border border-primary text-primary' : 'border border-white/10 text-slate-400'
      }`}
    >
